...
class RoyalGameOfUr:
   ...
   def load_board(self, board_file_name):
       """
       This function takes a file name and loads the map, creating BoardSquare\
       objects in a grid.
   ...
   def draw_block(self, output, i, j, square):
       ...
       for y in range(MAX_Y):
           for x in range(MAX_X):
               ...
               if square.rosette and (y, x) in [(1, 1), (1, MAX_X - 2), (MAX_Y \
                                                                         - 2, 1), (MAX_Y - 2, MAX_X - 2)]:
                   output[MAX_Y * i + y][MAX_X * j + x] = '*'
               if square.piece:
   ...
   def roll_d4_dice(self, n=4):
      """
      Keep this function as is.  It ensures that we'll have the same runs wit\
                           h different random seeds for rolls.
